...
Prefer a friendly chat window over the command line? A single command brings up
everything (Docker + Restack engine, Ollama, the agent worker, and the web
server) and opens a chat page — still 100% offline, no API key.

```bash
./start_offline_chat.sh
```

Then open **http://localhost:8000** and just type a question, for example
_“How does this project run agents fully offline?”_. The assistant answers in
plain language from your local files and shows which source files it used. Use
**New chat** to start a fresh conversation. The script is safe to re-run.

> Answers are generated by a local `llama3.1:8b` model on CPU, so each reply can
> take a couple of minutes; the page shows a “thinking…” indicator meanwhile.
